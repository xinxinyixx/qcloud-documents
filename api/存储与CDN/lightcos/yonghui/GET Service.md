...
该 API 接口不支持匿名请求，您需要使用帯 Authorization 签名认证的请求才能获取 Bucket 列表，且只能获取签名中 AccessID 所属根账户的 Bucket 列表。
...
```
GET / HTTP/1.1
Host: cos.shanghai.tce.cloud.yonghui.cn
Date: GMT Date
...
| Location           | ListAllMyBucketsResult.Buckets.Bucket | Bucket 所在地域。                                            | String |
